...
const page = async () => {
  ...
  //  im finding many and not one because find unique was erroring for me.
  const monthlyGoalQuery = await prisma.monthlyGoal.findMany({
    where: {
      userId: userId,
    },
  });
  console.log(monthlyGoalQuery);
  
  if (!monthlyGoalQuery || monthlyGoalQuery.length === 0) {
    redirect("/dashboard/monthly-goal");
  }

  const monthlyGoal = monthlyGoalQuery[0].amount;
  ...
  const sumMonthlyExpenses = monthlyExpenses.reduce(
    (acc, curr) => acc + curr.amount,
    0
  );
  const sumMonthlyIncome = monthlyIncome.reduce(
    (acc, curr) => acc + curr.amount,
    0
  );

  const progress = (sumMonthlyExpenses / monthlyGoal) * 100;

  const today = moment();
  const past5MonthStartDate = today
    .clone()
    .subtract(5, "months")
    .startOf("month")
    .toDate();
  const past5MonthEndDate = today
    .clone()
    .subtract(5, "months")
    .endOf("month")
    .toDate();
  const past4MonthStartDate = today
    .clone()
    .subtract(4, "months")
    .startOf("month")
    .toDate();
  const past4MonthEndDate = today
    .clone()
    .subtract(4, "months")
    .endOf("month")
    .toDate();
  const past3MonthStartDate = today
    .clone()
    .subtract(3, "months")
    .startOf("month")
    .toDate();
  const past3MonthEndDate = today
    .clone()
    .subtract(3, "months")
    .endOf("month")
    .toDate();
  const past2MonthStartDate = today
    .clone()
    .subtract(2, "months")
    .startOf("month")
    .toDate();
  const past2MonthEndDate = today
    .clone()
    .subtract(2, "months")
    .endOf("month")
    .toDate();
  const past1MonthStartDate = today
    .clone()
    .subtract(1, "months")
    .startOf("month")
    .toDate();
  const past1MonthEndDate = today
    .clone()
    .subtract(1, "months")
    .endOf("month")
    .toDate();

  const past1MonthMonthlyExpenses = await prisma.transaction.findMany({
    where: {
      userId,
      type: "expense",
      createdAt: {
        gte: past1MonthStartDate,
        lte: past1MonthEndDate,
      },
    },
  });

  const past1MonthExpenses = past1MonthMonthlyExpenses.reduce(
    (acc, curr) => acc + curr.amount,
    0
  );

  const past2MonthMonthlyExpenses = await prisma.transaction.findMany({
    where: {
      userId,
      type: "expense",
      createdAt: {
        gte: past2MonthStartDate,
        lte: past2MonthEndDate,
      },
    },
  });

  const past2MonthExpenses = past2MonthMonthlyExpenses.reduce(
    (acc, curr) => acc + curr.amount,
    0
  );
  const past3MonthMonthlyExpenses = await prisma.transaction.findMany({
    where: {
      userId,
      type: "expense",
      createdAt: {
        gte: past3MonthStartDate,
        lte: past3MonthEndDate,
      },
    },
  });
  const past3MonthExpenses = past3MonthMonthlyExpenses.reduce(
    (acc, curr) => acc + curr.amount,
    0
  );
  const past4MonthMonthlyExpenses = await prisma.transaction.findMany({
    where: {
      userId,
      type: "expense",
      createdAt: {
        gte: past4MonthStartDate,
        lte: past4MonthEndDate,
      },
    },
  });
  const past4MonthExpenses = past4MonthMonthlyExpenses.reduce(
    (acc, curr) => acc + curr.amount,
    0
  );
  const past5MonthMonthlyExpenses = await prisma.transaction.findMany({
    where: {
      userId,
      type: "expense",
      createdAt: {
        gte: past5MonthStartDate,
        lte: past5MonthEndDate,
      },
    },
  });
  const past5MonthExpenses = past5MonthMonthlyExpenses.reduce(
    (acc, curr) => acc + curr.amount,
    0
  );
  const past1MonthMonthlyIncome = await prisma.transaction.findMany({
    where: {
      userId,
      type: "income",
      createdAt: {
  ...
  const past1MonthIncome = past1MonthMonthlyIncome.reduce(
    (acc, curr) => acc + curr.amount,
    0
  );

  const past2MonthMonthlyIncome = await prisma.transaction.findMany({
    where: {
      userId,
      type: "income",
      createdAt: {
  ...
  const past2MonthIncome = past2MonthMonthlyIncome.reduce(
    (acc, curr) => acc + curr.amount,
    0
  );
  const past3MonthMonthlyIncome = await prisma.transaction.findMany({
    where: {
      userId,
      type: "income",
      createdAt: {
  ...
  const past3MonthIncome = past3MonthMonthlyIncome.reduce(
    (acc, curr) => acc + curr.amount,
    0
  );
  const past4MonthMonthlyIncome = await prisma.transaction.findMany({
    where: {
      userId,
      type: "income",
      createdAt: {
  ...
  const past4MonthIncome = past4MonthMonthlyIncome.reduce(
    (acc, curr) => acc + curr.amount,
    0
  );
  const past5MonthMonthlyIncome = await prisma.transaction.findMany({
    where: {
      userId,
      type: "income",
      createdAt: {
  ...
  const past5MonthIncome = past5MonthMonthlyIncome.reduce(
    (acc, curr) => acc + curr.amount,
    0
  );

  const last6MonthsExpenses = [
    { date: "5 months ago", Expense: past5MonthExpenses },
    { date: "4 months ago", Expense: past4MonthExpenses },
    { date: "3 months ago", Expense: past3MonthExpenses },
    { date: "2 months ago", Expense: past2MonthExpenses },
    { date: "1 months ago", Expense: past1MonthExpenses },
    { date: "this month ", Expense: sumMonthlyExpenses },
  ];
  const last6MonthsIncome = [
    { date: "5 months ago", Income: past5MonthIncome },
    { date: "4 months ago", Income: past4MonthIncome },
    { date: "3 months ago", Income: past3MonthIncome },
    { date: "2 months ago", Income: past2MonthIncome },
    { date: "1 months ago", Income: past1MonthIncome },
    { date: "this month ", Income: sumMonthlyIncome },
  ];
  ...
  return (
    <>
      <div>
        {!transactions || transactions.length === 0 ? ( <div className="  h-screen text-center">
        ...
        </div>) : (<div>  <PageTitle title="Dashboard" />
      <div className="rounded-lg border mt-12 p-12 flex justify-between bg-blue-500/15">
        <MonthlyGoalProgress
          goal={monthlyGoal}
          expenses={sumMonthlyExpenses}
          progress={progress}
        />
        <h4 className="text-green-500 text-xl">
          Your monthly income: {sumMonthlyIncome}$
        </h4>
      </div>
      <div className="rounded-lg border pb-8 mt-8   bg-blue-500/15">
        <ExpensesBarChart data={last6MonthsExpenses} />
      </div>
      <div className="rounded-lg border pb-8 mt-8   bg-blue-500/15 mb-20">
        <IncomeBarChart data={last6MonthsIncome} />
      </div></div>)}
      </div>
